...
public class tempAndroidServer {
	...
	public void run(int port) {
		try {
			...
			database.addMember(fileLocation, "hassaan", "hafeez", "6138996708");
				...
				if(dataReceived.contains("login:")){
					...
					if (entryData.length == 3) {
						...
						if (member != null && member.substring(member.lastIndexOf(" ")+1).equals(entryData[2])) {
							...
							DatagramPacket packet = new DatagramPacket(message, message.length, destination, receivePacket.getPort());
